...
class DDQN(Agent):
    def __init__(self,env,runner,lr,gamma,logger,memory_size,batch_size):
        super().__init__(env,logger,runner)
        ...
        # memory
        self.s_memory=RingBuf(size=memory_size)
        self.a_memory=RingBuf(size=memory_size)
        self.r_memory=RingBuf(size=memory_size)
        self.ns_memory=RingBuf(size=memory_size)
